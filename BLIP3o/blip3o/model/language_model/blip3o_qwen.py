...
class blip3oQwenForCausalLM(Qwen2_5_VLForConditionalGeneration, blip3oMetaForCausalLM):
    ...
    def _grid_no_shift_indices(self, H: int, W: int, N_reg: int):
        """
        CLS  -> (0, 0)
        REGs -> (0, 1..N_reg)
        PATCH-> rows 1..H, cols 1..W
        Grid size: H1 = H+1, W1 = W+1
        """
        H1, W1 = H + 1, W + 1
        def rc2i(r, c): return r * W1 + c
        idx_cls = rc2i(0, 0)
        idx_regs = [rc2i(0, 1 + k) for k in range(N_reg)]
        idx_patches = [rc2i(1 + r, 1 + c) for r in range(H) for c in range(W)]
        return H1, W1, idx_cls, idx_regs, idx_patches

    def _build_dino_grid_target_no_shift(
        self,
        T_cls_img: torch.Tensor,   # [B, C]
        T_reg_img: torch.Tensor,   # [B, C, N_reg]
        T_patch_img: torch.Tensor, # [B, C, H, W]
    ):
        """
        Returns:
        X     : [B, C, H+1, W+1] unified grid (CLS + regs + patches)
        valid : [B, 1, H+1, W+1] bool mask (True for valid sites)
        """
        B, C, H, W = T_patch_img.shape
        N_reg = T_reg_img.shape[2]
        H1, W1, _, _, _ = self._grid_no_shift_indices(H, W, N_reg)

        X = T_patch_img.new_zeros(B, C, H1, W1)
        valid = torch.zeros(B, 1, H1, W1, dtype=torch.bool, device=T_patch_img.device)
        ...
        # REGs
        X[:, :, 0, 1:1+N_reg] = T_reg_img
        valid[:, :, 0, 1:1+N_reg] = True
    ...
    def weighed_loss(self, img_vel_pred: torch.Tensor, target: torch.Tensor):
        m_cls, m_regs, m_patch = self.get_grid_masks(img_vel_pred)

        cls_loss   = F.mse_loss(img_vel_pred[m_cls].float(),   target[m_cls].float(),   reduction="mean")
        regs_loss  = F.mse_loss(img_vel_pred[m_regs].float(),  target[m_regs].float(),  reduction="mean")
        patch_loss = F.mse_loss(img_vel_pred[m_patch].float(), target[m_patch].float(), reduction="mean")
        print(
            f"loss/cls={cls_loss.item():.6f} | "
            f"loss/regs={regs_loss.item():.6f} | "
            f"loss/patch={patch_loss.item():.6f}"
        )

        w_cls   = getattr(self.get_model(), "lambda_cls",   0.3)
        w_regs  = getattr(self.get_model(), "lambda_regs",  0.3)
        w_patch = getattr(self.get_model(), "lambda_patch", 0.4)

        img_loss = w_cls * cls_loss + w_regs * regs_loss + w_patch * patch_loss

        def print_norms(name, preds, tgs):
            # Flattened selected entries
            # pred_sel = preds.float()
            # tgt_sel  = tgs.float()

            # elementwise norms
            # pred_elem_norm = pred_sel.norm(p=2) / (pred_sel.numel() ** 0.5)
            # tgt_elem_norm  = tgt_sel.norm(p=2) / (tgt_sel.numel() ** 0.5)

            pred_elem_norm = preds.norm(dim=1).mean(dim=(1,2)).mean()
            tgt_elem_norm  = tgs.norm(dim=1).mean(dim=(1,2)).mean()
            cosine_similarity = F.cosine_similarity(preds, tgs, dim=1, eps=1e-8).mean(dim=(1,2)).mean()

            print(
                f"[{name}] loss={F.mse_loss(preds, tgs, reduction='mean').item():.6f} | "
                f"pred_elem_norm={pred_elem_norm.item():.3f} | "
                f"tgt_elem_norm={tgt_elem_norm.item():.3f} | "
                f"cosine_similarity={cosine_similarity.item():.3f}"
            )

        print_norms("cls",   img_vel_pred[:,:,0,0].unsqueeze(-1).unsqueeze(-1), target[:,:,0,0].unsqueeze(-1).unsqueeze(-1))
        print_norms("regs",  img_vel_pred[:,:,0,1:1+self.get_model().dit.config.num_register_tokens].unsqueeze(-1), target[:,:,0,1:1+self.get_model().dit.config.num_register_tokens].unsqueeze(-1))
        print_norms("patch", img_vel_pred[:,:,1:,1:], target[:,:,1:,1:])

        return img_loss

    def get_grid_masks(self, img_vel_pred: torch.Tensor):
        B, C, H1, W1 = img_vel_pred.shape   # e.g., [B, 1024, 38, 38]
        device = img_vel_pred.device
        N_reg = self.get_model().dit.config.num_register_tokens  # you already have this
        m_cls = torch.zeros((B, 1, H1, W1), dtype=torch.bool, device=device)
        m_cls[:, :, 0, 0] = True 
        m_regs = torch.zeros((B, 1, H1, W1), dtype=torch.bool, device=device)
        m_regs[:, :, 0, 1:1+N_reg] = True 
        m_patch = torch.zeros((B, 1, H1, W1), dtype=torch.bool, device=device)
        m_patch[:, :, 1:, 1:] = True  
        # Expand to [B, C, H1, W1] so it matches img_vel_pred/target
        m_cls   = m_cls.expand(B, C, H1, W1)
        m_regs  = m_regs.expand(B, C, H1, W1)
        m_patch = m_patch.expand(B, C, H1, W1)

        return m_cls, m_regs, m_patch
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if self.predict_summary_token:
                ...
                assert latents is not None and cls_latents is not None and reg_latents is not None, "DINO grid latents not prepared correctly"
                assert latents.shape==(inputs_embeds.shape[0], 1024, 37, 37), f"Unexpected latents shape: {latents.shape}"
                assert cls_latents.shape==(inputs_embeds.shape[0], 1024), f"Unexpected cls_latents shape: {cls_latents.shape}"
                assert reg_latents.shape==(inputs_embeds.shape[0], 4, 1024), f"Unexpected reg_latents shape: {reg_latents.shape}"
            else:
                (
                    input_ids,
                    position_ids,
        ...
        if labels is not None:
            ...
            if latents is None:
                ...
                if not self.predict_dino_grid:
                    ...
                    lambda_sum = getattr(self.get_model(), "lambda_summary", 1.0)
                    ...
                    sigmas = self.get_sigmas(timesteps, latents.device, n_dim=latents.ndim, dtype=dtype)
                    noisy_latents = (1.0 - sigmas) * latents + sigmas * noise
                    assert noisy_latents.shape==(bsz, 1024, 37, 37), f"Unexpected noisy_latents shape: {noisy_latents.shape}"


                    B, C, H, W = latents.shape
                    # cls_latents = cls_latents.unsqueeze(-1).unsqueeze(-1)
                    noise_sum = torch.randn_like(cls_latents, device=latents.device)
                    sigmas_sum = self.get_sigmas(timesteps, latents.device, n_dim=noise_sum.ndim, dtype=latents.dtype)  # (B,1,1,1)
                    noisy_sum = (1.0 - sigmas_sum) * cls_latents + sigmas_sum * noise_sum

                    # reg_latents = reg_latents.unsqueeze(-1)
                    noise_regs = torch.randn_like(reg_latents, device=latents.device)
                    sigmas_regs = self.get_sigmas(timesteps, latents.device, n_dim=noise_regs.ndim, dtype=latents.dtype)  # (B,1,1,1)
                    noisy_regs = (1.0 - sigmas_regs) * reg_latents + sigmas_regs * noise_regs


                    dit_out, cls_out, regs_out = self.get_model().dit(
                        x=noisy_latents,
                        timestep=timesteps,
                        z_latents=self.mask_drop(img_hidden_states),
                        noisy_sum=noisy_sum,
                        noisy_regs=noisy_regs
                    )
                    assert dit_out.shape==(bsz, 1024, 37, 37), f"Unexpected dit_out shape: {dit_out.shape}"
                    assert cls_out.shape==(bsz, 1024), f"Unexpected cls_out shape: {cls_out.shape}"
                    assert regs_out.shape==(bsz, 4, 1024), f"Unexpected regs_out shape: {regs_out.shape}"
                    
                    target = noise - latents
                    sum_target = noise_sum - cls_latents
                    reg_target = noise_regs - reg_latents

                    img_vel_pred = dit_out


                    patch_loss = F.mse_loss(img_vel_pred.float(), target.float(), reduction="mean")
                    cls_loss = F.mse_loss(cls_out.float(), sum_target.float(), reduction="mean")
                    regs_loss = F.mse_loss(regs_out.float(), reg_target.float(), reduction="mean")

                    img_loss = 0.4 * patch_loss + 0.3 * cls_loss + 0.3 * regs_loss
                    print(f"patch loss {patch_loss}, cls loss {cls_loss}, regs loss {regs_loss}")
    ...
    def sample_images_cfg_dino(
        ...
        **kwargs,
        ...
        latent_size = self.get_model().dit.config.input_size 
        latent_channels = self.get_model().dit.config.in_channels
        num_register_tokens = self.get_model().dit.config.num_register_tokens
        ...
        cls_latents = randn_tensor(
            shape=(batch_size * num_images_per_prompt, latent_channels),
            generator=generator,
            device=device,      
            dtype=dtype,
        )
        reg_latents = randn_tensor(
            shape=(batch_size * num_images_per_prompt, num_register_tokens, latent_channels),
            generator=generator,
            device=device,
            dtype=dtype,
        )
        ...
        patch_scheduler = copy.deepcopy(scheduler)
        reg_scheduler = copy.deepcopy(scheduler)
        cls_scheduler = copy.deepcopy(scheduler)
        # scheduler.set_timesteps(num_inference_steps, sigmas=sigmas)

        patch_scheduler.set_timesteps(num_inference_steps, sigmas=sigmas)
        reg_scheduler.set_timesteps(num_inference_steps, sigmas=sigmas)
        cls_scheduler.set_timesteps(num_inference_steps, sigmas=sigmas)

        # Repeat z_latents and conditions for each image per prompt
        img_hidden_states_input = img_hidden_states_input.repeat_interleave(num_images_per_prompt, dim=0)

        for t in patch_scheduler.timesteps:
            # latent_model_input = latents.repeat(2, 1, 1, 1)
            latent_model_input = latents.repeat(2, 1, 1, 1)
            cls_latent_model_input = cls_latents.repeat(2, 1)
            reg_latent_model_input = reg_latents.repeat(2, 1, 1)

            if hasattr(scheduler, "scale_model_input"):
                latent_model_input = patch_scheduler.scale_model_input(latent_model_input, t)
                cls_latent_model_input = cls_scheduler.scale_model_input(cls_latent_model_input, t)
                reg_latent_model_input = reg_scheduler.scale_model_input(reg_latent_model_input, t)

            # predict noise model_output
            noise_pred, cls_pred, reg_pred = self.get_model().dit(
                x=latent_model_input,
                timestep=t.unsqueeze(0).expand(latent_model_input.shape[0]).to(latent_model_input.device, torch.long),
                z_latents=img_hidden_states_input,
                noisy_sum=cls_latent_model_input,
                noisy_regs=reg_latent_model_input,
            )

            noise_pred_uncond, noise_pred = noise_pred.chunk(2)
            noise_pred = noise_pred_uncond + guidance_scale * (noise_pred - noise_pred_uncond)

            # cls_pred = cls_pred.unsqueeze(-1).unsqueeze(-1)
            cls_pred_uncond, cls_pred = cls_pred.chunk(2, dim=0)
            cls_pred = cls_pred_uncond + guidance_scale * (cls_pred - cls_pred_uncond)

            # reg_pred = reg_pred.transpose(1,2).unsqueeze(-1)
            reg_pred_uncond, reg_pred = reg_pred.chunk(2, dim=0)
            reg_pred = reg_pred_uncond + guidance_scale * (reg_pred - reg_pred_uncond)

            # compute previous image: x_t -> x_t-1
            latents = patch_scheduler.step(noise_pred, t, latents).prev_sample
            cls_latents = cls_scheduler.step(cls_pred, t, cls_latents).prev_sample
            reg_latents = reg_scheduler.step(reg_pred, t, reg_latents).prev_sample
        # samples = self.decode_latents(latents, return_tensor=return_tensor)
        # breakpoint()

        assert reg_latents.shape[1] == 4, f"Expected reg_latents to have shape[2] == 4, but got {reg_latents.shape}"
        assert cls_latents.shape[1] == 1024, f"Expected cls_latents to have shape[2] == 1024, but got {cls_latents.shape}"
        assert latents.shape[1] == 1024, f"Expected latents to have shape[1] == 1024, but got {latents.shape}"  
        assert latents.shape[2] == 37, f"Expected latents to have shape[2] == 37, but got {latents.shape}"
        assert latents.shape[3] == 37, f"Expected latents to have shape[3] == 37, but got {latents.shape}"  
        assert reg_latents.shape[2] == 1024, f"Expected reg_latents to have shape[1] == 1024, but got {reg_latents.shape}"  

        return cls_latents, reg_latents, latents


    @torch.no_grad()
    def sample_images_no_cfg_dino(
        self,
        img_hidden_states,
        scheduler,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        ...
        **kwargs,
        ...
        for t in patch_scheduler.timesteps:
            # latent_model_input = latents.repeat(2, 1, 1, 1)
            latent_model_input = latents
            cls_latent_model_input = cls_latents
            reg_latent_model_input = reg_latents
    ...
    def evaluate_mapper_dino(
        ...
        sum_cos_means, sum_cos_stds, sum_mse_vals = [], [], []
        sum_pred_norm_means, sum_pred_norm_stds = [], []
        sum_tgt_norm_means, sum_tgt_norm_stds = [], []
        sum_ratio_means, sum_ratio_stds = [], []

        reg_cos_means, reg_cos_stds, reg_mse_vals = [], [], []
        reg_pred_norm_means, reg_pred_norm_stds = [], []
        reg_tgt_norm_means, reg_tgt_norm_stds = [], []
        reg_ratio_means, reg_ratio_stds = [], []
        ...
        for batch in tqdm(dataloader, desc="Eval mapper"):
            ...
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels, latents, sum_latents, reg_latents) = self.prepare_inputs_labels_for_multimodal(
                input_ids,
                position_ids,
                attention_mask,
                past_key_values,
            ...
            scheduler = self._inference_scheduler
            summary_pred, reg_pred, pred = self.sample_images_cfg_dino(
                    img_hidden_states,
                    scheduler=scheduler,
                    num_inference_steps=num_inference_steps,
                    num_images_per_prompt=1,
            ...
            assert len(summary_pred.shape) == 2, f"Expected summary_pred to have shape [B, _], but got {summary_pred.shape}"
            assert summary_pred.shape[1] == 1024, f"Expected summary_pred to have shape [B, 1024], but got {summary_pred.shape}"
            summary_pred = summary_pred.float()
            sum_target_f = sum_latents.float()

            assert len(reg_pred.shape) == 3, f"Expected reg_pred to have shape [B, num_register_tokens, _], but got {reg_pred.shape}"
            assert reg_pred.shape[2] == 1024, f"Expected reg_pred to have shape [B, {reg_latents.shape[1]}, _], but got {reg_pred.shape}"
            assert reg_pred.shape[1] == 4, f"Expected reg_pred to have shape [B, _, {reg_latents.shape[2]}], but got {reg_pred.shape}"
            reg_pred_f = reg_pred.float()
            reg_target_f = reg_latents.float()
            ...
            sum_cos_map = F.cosine_similarity(summary_pred, sum_target_f, dim=1, eps=1e-8)
            sum_cos_means.append(sum_cos_map.mean().item())  # avg of per-image means
            sum_mse_vals.append(F.mse_loss(summary_pred, sum_target_f, reduction="mean").item())
            sum_pred_norm = summary_pred.norm(dim=1)     # [B]
            sum_target_norm = sum_target_f.norm(dim=1) # [B]
            sum_ratio = sum_pred_norm / (sum_target_norm + 1e-8)
            sum_pred_norm_means.append(sum_pred_norm.mean().item())
            sum_tgt_norm_means.append(sum_target_norm.mean().item())
            sum_ratio_means.append(sum_ratio.mean().item())

            reg_cos_map = F.cosine_similarity(reg_pred_f, reg_target_f, dim=2, eps=1e-8)  # [B, num_register_tokens]
            reg_cos_means.append(reg_cos_map.mean(dim=1).mean().item())  # avg of per-image means
            reg_cos_stds.append(reg_cos_map.std(dim=1).mean().item())  # avg of per-image stds
            reg_mse_vals.append(F.mse_loss(reg_pred_f, reg_target_f, reduction="mean").item())
            
            reg_pred_norm = reg_pred_f.norm(dim=2)     # [B, num_register_tokens]
            reg_target_norm = reg_target_f.norm(dim=2) # [B, num_register_tokens]
            reg_ratio = reg_pred_norm / (reg_target_norm + 1e-8)

            reg_pred_norm_means.append(reg_pred_norm.mean(dim=1).mean().item())
            reg_pred_norm_stds.append(reg_pred_norm.std(dim=1).mean().item())
            reg_tgt_norm_means.append(reg_target_norm.mean(dim=1).mean().item())
            reg_tgt_norm_stds.append(reg_target_norm.std(dim=1).mean().item())
            reg_ratio_means.append(reg_ratio.mean(dim=1).mean().item())
            reg_ratio_stds.append(reg_ratio.std(dim=1).mean().item())
        ...
        metrics.update({
            "reg_cos_mean":         sum(reg_cos_means) / len(reg_cos_means),
            "reg_mse":              sum(reg_mse_vals) / len(reg_mse_vals),
            "reg_norm_ratio_mean":  sum(reg_ratio_means) / len(reg_ratio_means),
            "reg_pred_norm_mean":   sum(reg_pred_norm_means) / len(reg_pred_norm_means),
            "reg_target_norm_mean": sum(reg_tgt_norm_means)  / len(reg_tgt_norm_means),
            "reg_cos_std":          sum(reg_cos_stds) / len(reg_cos_stds),
            "reg_norm_ratio_std":   sum(reg_ratio_stds) / len(reg_ratio_stds),
            "reg_pred_norm_std":    sum(reg_pred_norm_stds) / len(reg_pred_norm_stds),
            "reg_target_norm_std":  sum(reg_tgt_norm_stds)  / len(reg_tgt_norm_stds),
        })

        metrics["n_images"] = int(sample_count)
    ...
    def generate_embeddings_dino(
        ...
        if cfg:
            ...
            summary_pred, reg_pred, pred = self.sample_images_no_cfg_dino(
                    img_hidden_states,
                    scheduler=scheduler,
                    num_inference_steps=num_inference_steps,
                    num_images_per_prompt=1,
        ...
        return (summary_pred, reg_pred, pred), metrics
