...
class blip3oMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.config.predict_summary_token = model_args.predict_summary_token
        self.config.predict_dino_grid = model_args.predict_dino_grid
...
class blip3oMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if self.config.predict_summary_token:
            prompt_image_embeds, summary_embed = gen_vision_tower(gen_images) # TODO: check dimension
        else:
            prompt_image_embeds = gen_vision_tower(gen_images) # TODO: check dimension

        if self.config.predict_dino_grid:
            # Seperate the CLS token, Register tokens, and Patch tokens
            num_register_tokens = self.config.num_register_tokens
            assert num_register_tokens==4, "num_register_tokens should be 4"
            cls_image_embed, reg_image_embed, prompt_image_embeds = prompt_image_embeds[:, 0, :], prompt_image_embeds[:, 1:num_register_tokens+1, :], prompt_image_embeds[:, 1+num_register_tokens:, :]
        ...
        if self.config.predict_dino_grid:
            b, n, c = prompt_image_embeds.shape
            target_cls_embed = torch.clone(cls_image_embed).detach()
            target_reg_embed = torch.clone(reg_image_embed).detach()
            sqrt_n = int(n**0.5)
            prompt_image_embeds = prompt_image_embeds.permute(0, 2, 1).view(b, c, sqrt_n, sqrt_n)

        target_image_embeds = torch.clone(prompt_image_embeds).detach()

        if self.config.predict_summary_token:
            target_summary_embed = torch.clone(summary_embed).detach()
        ...
        if self.config.predict_summary_token:
            return None, position_ids, attention_mask, past_key_values, text_embeds, labels, target_image_embeds, target_summary_embed
        elif self.config.predict_dino_grid:
            return None, position_ids, attention_mask, past_key_values, text_embeds, labels, target_image_embeds, target_cls_embed, target_reg_embed
        else:
