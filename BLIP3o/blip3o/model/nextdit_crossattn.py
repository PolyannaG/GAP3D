...
class NextDiTCrossAttnConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        summary_token_dim: Optional[int] = None,
...
class NextDiTCrossAttn(PreTrainedModel):
    ...
    def __init__(
        ...
        self.model = LuminaNextDiT2DModel(
            ...
            summary_token_dim=config.summary_token_dim,
    ...
    def forward(self, x, timestep, z_latents, noisy_sum=None, noisy_regs=None, **kwargs):
        if self.model.summary_token_dim is None and self.model.num_register_tokens is None:
            ...
            return model_pred
        elif self.model.summary_token_dim is None and self.model.num_register_tokens is not None:
            model_pred, cls_pred, reg_pred = self.model(
                hidden_states=x,
                timestep=timestep,
                encoder_hidden_states=z_latents,
                encoder_mask=torch.ones((z_latents.shape[0], z_latents.shape[1]), device=z_latents.device),
            ...
            return model_pred.sample, cls_pred, reg_pred
        else:
            model_pred, summary_token = self.model(
                hidden_states=x,
                timestep=timestep,
                encoder_hidden_states=z_latents,
                encoder_mask=torch.ones((z_latents.shape[0], z_latents.shape[1]), device=z_latents.device),
            ...
            return model_pred.sample, summary_token
