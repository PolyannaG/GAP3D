...
def _render(file_path, sha256, output_dir, num_views, save_depth=False):
    output_folder = os.path.join(output_dir, 'renders', sha256)

    if num_views == 4:
        yaws = [0, np.pi/2, np.pi, 3*np.pi/2]
        pitchs = [30/180*np.pi] * 4
        radius = [2] * 4
        fov = [40/180*np.pi] * 4
        ...
        # Generate camera poses - evenly distributed around object
        # Using circular arrangement with slight elevation (like Point-E)
        yaws = [i * (2 * math.pi / num_views) for i in range(num_views)]
        pitchs = [math.radians(30)] * num_views  # 30 degrees elevation
        radius = [2] * num_views
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_depth', action='store_true', help='Save depth maps for point cloud evaluation')
    ...
    # process objects
    func = partial(_render, output_dir=opt.output_dir, num_views=opt.num_views, save_depth=opt.save_depth)
