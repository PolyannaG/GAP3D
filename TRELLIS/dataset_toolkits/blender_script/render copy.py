...
def init_render(engine='CYCLES', resolution=512, geo_mode=False):
    ...
    bpy.context.scene.render.film_transparent = False            # << solid background
...
def force_black_world_background():
    ...
    # get/create required nodes
    out = next((n for n in nodes if n.type == 'OUTPUT_WORLD'), nodes.new('ShaderNodeOutputWorld'))
    bg  = next((n for n in nodes if n.type == 'BACKGROUND'),     nodes.new('ShaderNodeBackground'))

    # disconnect existing inputs to output.surface
    for link in list(out.inputs['Surface'].links):
        links.remove(link)

    # black background, strength 1
...
def init_lighting():
    ...
    # --- Top (key) AREA light: big & soft ---
    top_light_data = bpy.data.lights.new("Top_Area", type='AREA')
    top_light_data.shape = 'SQUARE'     # single size controls both axes
    top_light_data.size = 12.0          # big light => soft shadows
    top_light_data.energy = 2500.0      # tweak if your renders look too dim/bright
    top_light_data.color = (1.0, 1.0, 1.0)
    # Better sampling for area lights
    top_light_data.cycles.use_multiple_importance_sampling = True

    top_light = bpy.data.objects.new("Top_Area", top_light_data)
    bpy.context.collection.objects.link(top_light)
    top_light.location = (0.0, 0.0, 6.0)   # above the normalized unit cube
    top_light.rotation_euler = (0.0, 0.0, 0.0)
    top_light.scale = (1.0, 1.0, 1.0)      # use size on light data, not object scale

    # --- Off-axis (fill) AREA light: also large, slightly angled ---
    fill_light_data = bpy.data.lights.new("Fill_Area", type='AREA')
    fill_light_data.shape = 'RECTANGLE'
    fill_light_data.size = 8.0            # width (X)
    fill_light_data.size_y = 4.0          # height (Y)
    fill_light_data.energy = 1200.0
    fill_light_data.color = (1.0, 1.0, 1.0)
    fill_light_data.cycles.use_multiple_importance_sampling = True

    fill_light = bpy.data.objects.new("Fill_Area", fill_light_data)
    bpy.context.collection.objects.link(fill_light)
    fill_light.location = (4.0, -4.0, 3.0)                      # off-axis position
    fill_light.rotation_euler = (math.radians(-35), math.radians(45), 0.0)

    # Return handles if you want to tweak later
    return {"top_light": top_light, "fill_light": fill_light}
...
def main(arg):
    ...
    force_black_world_background() 
    set_color_management_standard() 
